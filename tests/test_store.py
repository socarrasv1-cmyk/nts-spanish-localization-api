...
def test_mutate_persists_review_workflow_changes(tmp_path):
    store = PersistentStore(data_dir=str(tmp_path), database_url="")
    store.save("translation_memory", {"proposals": []})

    store.mutate(
        "translation_memory",
        lambda data: data["proposals"].append(
            {"proposal_id": "proposal-456", "status": "proposed"}
        ),
    )

    restarted = PersistentStore(data_dir=str(tmp_path), database_url="")
    assert restarted.load("translation_memory")["proposals"][0]["proposal_id"] == "proposal-456"
